	...
	return (
		<div className="goods__detail-wrap">
			<Row>
				...
				{dynProps && dynProps.length > 0 && (
					<>
						...
						<Row gutter={[16, 10]}>
							{dynProps?.map((item, i) => {
								let type = item.type;
								let value = item?.categoryPropertyValues ? item?.categoryPropertyValues?.toString() : '';
								if (type === 4) {
									value = value.split(',');
								}
								if (type === 8) {
								...
								return (
									<Col span={8} key={i}>
										<p className="u-els" title={value}>
											<span className="u-c888">{item.categoryPropertyName}：</span>
											{value}
										</p>
									</Col>
								);
				...
				<Col span={24}>
					<Table columns={columns} dataSource={saleProperties} rowKey={'skuId'}></Table>
				</Col>
			</Row>
		</div>
	);
