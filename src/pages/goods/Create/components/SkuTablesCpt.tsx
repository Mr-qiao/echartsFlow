...
const SkuTablesCpt: any = React.forwardRef(
  ({ value, onChange }: any, ref): any => {
        ...
        component: (record: any, idx: number) => {
          return (
            <>
              ...
              />
              {!dataSource[idx][record.dataIndex] && (
                <p style={{ color: '#ff4d4f' }}>请输入</p>
              )}
            </>
          );
        },
    ...
    const handleBatchDel = () => {
      if (selectedRowKeys.length <= 0) {
        ...
        let _ = [...dataSource];
        let arr = _.filter(
          (item) => !selectedRowKeys.includes(item.skuId as never),
        );
        serDataSource(arr);
        triggerChange(arr);
      }
        ...
        <th {...props}>
          {titleType && titleType === 'input' ? (
            <div style={{ display: 'flex', alignItems: 'center' }}>
              {restProps.rules && (
                <span style={{ marginRight: 5, color: '#f00009' }}>*</span>
              )}
              <Input
                value={inputValue}
