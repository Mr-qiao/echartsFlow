...
const BasicInfo: React.FC<IStepProps> = ({ sampleInfo, proofInfo, onOk }) => {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      ...data,
      sampleDesignerId: {
        label: proofInfo.sampleDesignerName,
        proofInfo: data.sampleDesignerId,
      },
      ...shouldDateTransformKeys.reduce((acc: Recordable<any>, cur) => {
        if (data[cur]) {
          acc[cur] = dayjs(data[cur]);
        }
        return acc;
      }, {}),
      skcList:
  ...
  const handleSave = async () => {
    try {
      ...
      const data = {
        ...
        ...shouldDateTransformKeys.reduce((acc: Recordable<any>, cur) => {
          if (values[cur]) {
            acc[cur] = dayjs(values[cur]).format('YYYY-MM-DD');
          }
          return acc;
        }, {}),
    ...
    return [
      {
        ...
        render: (_, record, index: number) => {
          ...
          const options = sampleInfo.colorComb.map((color) => ({
            label: color,
            value: color,
          }));
          return (
      ...
      },
      {
        ...
        render: (_, record, index: number) => {
          const field = fields[index];
          const options = sampleInfo.sizeComb.map((size) => ({
            label: size,
            value: size,
          }));
          return (
  ...
  return (
    <Form form={form} labelCol={{ style: { width: 100 } }} disabled>
      <Descriptions labelStyle={{ width: 100, justifyContent: 'flex-end' }}>
        ...
        <Descriptions.Item label="样衣编号">
          {sampleInfo.sampleClothesCode ?? '-'}
        </Descriptions.Item>
        <Descriptions.Item label="样衣名称">
          {sampleInfo.title}
        </Descriptions.Item>
        <Descriptions.Item label="品牌">
          {sampleInfo.brandName ?? '-'}
        </Descriptions.Item>
        ...
        <Descriptions.Item label="设计师">
          {sampleInfo.designerName ?? '-'}
        </Descriptions.Item>
        <Descriptions.Item label="供应商">
          {proofInfo.sampleClothesSupplierName || '-'}
        </Descriptions.Item>
