...
const ImageCpt: React.FC<IProps> = (options: any) => {
	const {value, onChange, thumbnailSize, customText = false, tip = true, itemRender = false} = options
	const props: any = {
		listType: "picture-card",
		tip: tip && `最多上传${options?.categoryPropertyRule?.max || 1}
		张图片，支持支持JPG，JPEG，PNG格式，每张图片小于${options?.categoryPropertyRule?.size || 5}M`,
		value,
		accept: 'image/jpg,image/png,image/jpeg,image/gif',
		maxCount: options?.categoryPropertyRule?.max || 1,
