...
function QuotationEdit() {
  ...
  // 图样附图
  const arr =
    data?.drawingMap?.accessoryImages?.length > 0
      ? data?.drawingMap.accessoryImages.map((item: any) => ({ src: item }))
      : [];
  // 其他附件
  const qtarr =
    data?.drawingMap?.accessoryFiles?.length > 0
      ? data?.drawingMap?.accessoryFiles
      : [];
  // 尺寸附图
  const ccftarr =
        ...
        return (
          <Select
            ...
            onChange={(e) => {
              ...
              setDataSourceQt(NewArr);
              const asd = NewArr.filter(
                (item) => item.bjsxgg === recode.bjsxgg,
              );
          ...
          >
            {data?.itemSkuList?.map((item: any) => {
              return <Option key={item.skuId}>{item.properties}</Option>;
            })}
          </Select>
        );
      },
        ...
        return (
          <a
            onClick={() => {
              ...
              const sumby = _.sumBy(asd, 'qthz');
              const minbyhuizong =
                _.ceil(_.minBy(NewArr, 'qthz')?.qthz, 2) || 0;
              const maxbyhuizong =
                _.ceil(_.maxBy(NewArr, 'qthz')?.qthz, 2) || 0;
        ...
        <Descriptions column={1}>
          ...
          <Descriptions.Item label={'尺寸标准'}>
            <Table
              ...
              scroll={{ x: 900 }}
              columns={columns}
              dataSource={data?.sizeMap?.sizeDetailList}
            />
          </Descriptions.Item>
